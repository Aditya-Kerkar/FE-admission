...
app.get('/FEadmissiondata', (req, res) => {
  const query = 'SELECT id, fullname, documentsApproved, transactionproofStatus, class FROM user_details_admission1';
...
app.get('/docverificationFEAdmission/:uid', (req, res) => {
  const userId = req.params.uid;
  console.log(userId);
  const query = `SELECT id, fullname, email, mobile_number, annual_income, category, cet_application_id, photo, marksheet10, leavingCertificate12, marksheet12, cetMarksheet, jeeMarksheet, signature, domicilecert, castecertificate, castevalidity, noncreamylayer, income, other, cbse_admit_card_id, fcverificationcopy, capAllotmentLetter, fcverificationcopyStatus, capAllotmentLetterStatus,cbse_admit_card_idStatus,photoStatus, leavingCertificate12Status, marksheet10Status, marksheet12Status, cetMarksheetStatus, jeeMarksheetStatus, signatureStatus, domicilecertStatus, castecertificateStatus, castevalidityStatus, noncreamylayerStatus, incomeStatus, otherStatus, transactionproofStatus, documentsApproved FROM user_details_admission1 WHERE id = '${userId}';`;
...
app.get('/getFeeDetailsFE/:id',(req,res)=>{
  ...
  const query = util.promisify(db.query).bind(db);
    async function performQueries(uid) {
        ...
        try {
          // First query
          const query1 = `SELECT preference, fullname, year_of_admission FROM user_details_admission1 WHERE id = ${uid} ; `;
          const result1 = await query(query1, uid);
          if (result1.length > 0) {
            // dataToSend = result1;
